...
const AuthOptions={
    callbackURL:'/auth/google/callback',//THIS IS THE URL THAT GOOGLE SENDS THE AUTHORIZATION CODE TO
...
server.use(cookie_session({
    ...
    maxAge:1000*60*60, //max Duration till the Session is Alive (1 min in this case)
...
function checkLoggedIn(req,res,next){
    if(req.user){
        ...
        res.status(401).send('Pls Log In First')
    }
    
}
server.get('/secret',checkLoggedIn,(req,res)=>{
    let string="You are logged in as "+String(req.user.id)
